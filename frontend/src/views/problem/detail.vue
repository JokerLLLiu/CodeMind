    ...
    <div>
        <el-page-header @back="goBack">
            <template #content>
                ...
                <span><el-tag
                        :type="problemInfo.difficulty === 1 ? 'success' : problemInfo.difficulty === 2 ? 'warning' : 'danger'">
                        {{ problemInfo.difficulty === 1 ? '简单' : problemInfo.difficulty === 2 ? '中等' : '困难' }}
                    </el-tag class="tagU"></span>
                <span><el-tag v-for="tag in problemInfo.tags" :key="tag" :type="primary" effect="light" round
                        class="tag">{{ tag
                        }}</el-tag></span>
            </template>
        </el-page-header>
                    ...
                    <div class="collapse">
                        <el-collapse v-model="activeNames" @change="handleChange">
                            <el-collapse-item title="题目描述" name="1" class="demo">
                                <div>
                                    {{ problemInfo.description }}<br>nihao
                                </div>
                            </el-collapse-item>
